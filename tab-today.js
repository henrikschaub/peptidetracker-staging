...
function showDayInline(date,tileEl){
  ...
  if(isPast){
    ...
    _getDynamicEnhancedDoses(d,true).forEach(function(x){if(!doses.some(function(y){return y.id===x.id;}))doses.push(x);});
  ...
  doses.forEach(function(dose){
    ...
    var _sdi=realId.substring(0,realId.lastIndexOf('_')).replace(/-am$|-pm$/,'');var _sdp=PEPTIDE_CAT.find(function(c){return c.id===_sdi;});if(_sdp){var _sdib=document.createElement('button');_sdib.className='info-btn';_sdib.textContent='ℹ';(function(_id){_sdib.onclick=function(e){e.stopPropagation();showPeptideCard(_id);};}(_sdi));item.appendChild(_sdib);}else if(dose.compId){var _sdce=(ENHANCEMENT_COMPOUNDS||[]).find(function(c){return c.id===dose.compId;});if(_sdce){var _sdib=document.createElement('button');_sdib.className='info-btn';_sdib.textContent='ℹ';(function(_id){_sdib.onclick=function(e){e.stopPropagation();showEnhancedCard(_id);};}(dose.compId));item.appendChild(_sdib);}}
