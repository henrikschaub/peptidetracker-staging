  ...
  if(trt.enabled){
    ...
    TRT_CAT.forEach(function(c){
      ...
      if(isSel&&selData){
        ...
        if(!isNebido){html+='<div class="cfg-row"><div class="cfg-lbl">Days</div><div class="day-chips">'+DAYS_SHORT.map(function(lbl,di){return'<div class="day-chip'+((selData.days||[]).includes(di)?' sel':'')+'" onclick="wizSetTRTDays(\''+c.id+'\','+di+')">'+lbl+'</div>';}).join('')+'</div></div>';}
